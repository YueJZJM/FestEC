...
public class AutoPhotoLayout extends LinearLayoutCompat {
    ...
    private AppCompatImageView mTargeImageView = null;
    ...
    public final void onCropTarget(Uri uri) {
        ...
        Glide.with(mDelegate)
                .load(uri)
                .apply(OPTIONS)
                .into(mTargeImageView);
    }

    private void createNewImageView() {
        mTargeImageView = new AppCompatImageView(getContext());
        mTargeImageView.setId(mCurrentNum);
        mTargeImageView.setLayoutParams(mParams);
        mTargeImageView.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View view) {
                //获取要删除的图片
                ...
                if (window != null) {
                    ...
                    window.findViewById(R.id.dialog_image_clicked_btn_delete)
                            .setOnClickListener(new OnClickListener() {
                                ...
                                public void onClick(View view) {
                                    //得到要删除的图片
                                    final AppCompatImageView deleteImageViwe = findViewById(mDeleteId);
                                    //设置图片逐渐消失的动画
        ...
        //添加子view的时候传入位置
        this.addView(mTargeImageView, mCurrentNum);
        mCurrentNum++;
